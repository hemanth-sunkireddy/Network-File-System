...
int nm_sock = 0; // Naming Server socket (global for signal handling)
int ss_sock = 0; // Storage Server’s own listening socket
bool running = true;
...
int main()
{
    signal(SIGINT, handleExit); // Handle Ctrl+C
    ...
    string handshake = "Storage Server";
    send(nm_sock, handshake.c_str(), handshake.size(), 0);
    cout << "[→] Sent handshake to Naming Server: " << handshake << endl;
    ...
    cout << "[*] Storage Server is now registered and active." << endl;
    cout << "==============================\n";
    ...
    {
        memset(buffer, 0, sizeof(buffer));
        ssize_t bytes = read(nm_sock, buffer, sizeof(buffer));
        if (bytes > 0)
        {
            cout << "[←] Message from Naming Server: " << buffer << endl;
        }
        else if (bytes == 0)
        {
            cout << "[x] Connection closed by Naming Server." << endl;
            break;
        }
        else
        {
        ...
        // Optional: send heartbeat or ACK periodically
        string heartbeat = "Storage heartbeat ACK";
        send(nm_sock, heartbeat.c_str(), heartbeat.size(), 0);
        sleep(5);
    }
